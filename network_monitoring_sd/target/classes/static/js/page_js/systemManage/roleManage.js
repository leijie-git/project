...
//    var roleType = $("#roleType").val();
    ...
    var edit = $("#edit").val();
    if(edit =="0"){
    	var url = baseUrl + '/role/addRole?type=0';
    }else{
    	var url = baseUrl + '/role/updateRole?type=0';
    }
...
$(".deleteMore").click(function () {
    ...
    var url = baseUrl + "/role/deleteManySystemRole?type=0";
    $.ajax({
        type: "get",
        async: true,
        beforeSend: function(request){
...
function editRole(_this) {
    ...
    $("#indexId").val(indexId);
    $("#newAdd").modal();
    var url = baseUrl + '/role/editRole?type=0';
    $.ajax({
        type: "get",
        async: true,
        beforeSend: function(request){
...
function deleteRole(_this) {
    var indexId = _this.id;
    var url = baseUrl + '/role/deleteRole?type=0';
    layer.open({
        content: "是否确定删除？",
        btn: ["确认", "取消"],
...
$(".btnGrant").click(function () {
    ...
    if (checkvalues == "") {
        ...
        var url = baseUrl + "/roleManage/authorizeSystemUserRole?type=0";
        layer.open({
...
function initTreeDataSource() {
    $.ajax({
        url: baseUrl + "/resource/resourceList?type=0",
        data: {},
        beforeSend: function(request){
            request.setRequestHeader("Authorization","Bearer "+login.token);
        },
...
function mainSearch() {
    $('#roleTable').bootstrapTable({
        url: baseUrl + '/role/roleList?type=0',         //请求后台的URL（*）
        method: 'get',                      //请求方式（*）
...
function initGrant() {
    ...
    $('#grantTable').bootstrapTable({
        url: baseUrl + '/personManage/employeeManage/findAllEmployee?type=0',         //请求后台的URL（*）
        method: 'post',                      //请求方式（*）
        contentType: 'application/x-www-form-urlencoded',
        toolbar: '#toolbar',                //工具按钮用哪个容器
        striped: true,                      //是否显示行间隔色
