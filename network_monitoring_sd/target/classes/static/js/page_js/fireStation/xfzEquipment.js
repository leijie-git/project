$(function () {
    ...
    $(".newAdd").click(function () {
        ...
        //$(".password").show();
        $("#newEmploy .modal-title").html("新增消防设施");
        $("#newEmploy .btnSure").show();
        //$("#continue").show();
        
        $("#newXFZEquipment-equipmentname").val("");
        $("#newXFZEquipment-equipmentcount").val("");
        $("#newXFZEquipment-unit").val("");
        $("#newXFZEquipment-unitid").val("");
    ...
    /*$("#continue").click(function(){
        var equipmentname = $("#newXFZEquipment-equipmentname").val();
        var equipmentcount = $("#newXFZEquipment-equipmentcount").val();
        var unit = $("#newXFZEquipment-unit").val();
        var unitid = $("#newXFZEquipment-unitid").val();
    ...
    $(".btnSure").click(function () {
        ...
        if (editId == 0) {
            ...
            var url = baseUrl + "/equipment/update";
            var data = {
            		id:id,
            		equipmentname:equipmentname,
            		equipmentcount:equipmentcount,
...
function editBaseInfo(_this) {
	...
	$("#newEmploy").modal("show");
    $("#newEmploy .modal-title").html("编辑消防设施");
    $("#newEmploy .btnSure").show();
    //$("#continue").hide();
    var id = _this.id;
    $("#indexId").val(id);
    $("#editId").val("1");
    $("#newXFZEquipment-equipmentname").val(_this.equipmentname);
    $("#newXFZEquipment-unitid").val(_this.unitid);
    $("#newXFZEquipment-equipmentcount").val(_this.equipmentcount);
    $("#newXFZEquipment-unit").val(_this.unit);
}
//查看
function viewBaseInfo(_this) {
	$("#newEmployView").modal("show");
	$("#newXFZEquipmentView-equipmentname").val(_this.equipmentname);
    $("#newXFZEquipmentView-unitid").val(_this.unitid);
    $("#newXFZEquipmentView-equipmentcount").val(_this.equipmentcount);
    $("#newXFZEquipmentView-unit").val(_this.unit);
...
function mainSearch() {
    $('#userTable').bootstrapTable({
        url: baseUrl + '/equipment/getList',         //请求后台的URL（*）
        method: 'get',                      //请求方式（*）
        ...
        success: function (data) {
        	if(data.success){
				for(var i=0;i<data.obj.length;i++){
					$("#newXFZEquipment-unitid").append('<option value="'+data.obj[i].id+'">'+data.obj[i].unitname+'</option>');
					$("#newXFZEquipmentView-unitid").append('<option value="'+data.obj[i].id+'">'+data.obj[i].unitname+'</option>');
            	}
			}
        }
    })
