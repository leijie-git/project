...
function mainSearch() {
    var url = baseUrl + "/resource/resourceList?type=1";
...
$(".btnAdd").click(function () {
    ...
    if (editId == 0) {
        ...
        url = baseUrl + "/resource/addResource?type=1";
    } else {
        if (id == "") {
            layer.msg("编号不能为空！");
            return;
        ...
        url = baseUrl + "/resource/updateResource?type=1";
    }
    $.ajax({
        type: "post",
        async: true,
...
function editResource(indexId) {
    ...
    var url = baseUrl + "/resource/editResource?type=1";
    $.ajax({
        type: "get",
        async: true,
        beforeSend: function(request){
...
function deleteResource(_this) {
    var indexId =_this;
    var delete_url = baseUrl + "/resource/deleteResource?type=1";
    layer.open({
        content: '是否确定删除？',
        btn: ["确认", "取消"],
        yes: function () {
