...
! function($) {
    ...
    MMGrid.prototype = {
        ...
        _initLayout: function($el) {
            ...
            var mmGrid = [
                ...
                '<span class="mmg-message">' + this.opts.noDataText + '</span>',
            ...
            if (opts.checkCol) {
                var renFun;
                if ("checkbox" === opts.cols[0].title) {
                    renFun = opts.cols.shift().renderer;
                };
                renFun = renFun || function() {
        ...
        _titleHtml: function(col, rowspan) {
            ...
            if (!col.cols) {
                ...
                if (col.isCbx !== true) {
                    var title = col.titleHtml ? col.titleHtml : col.title;
                    titleHtml.push('<span title="'+ title +'" class="mmg-title');
                } else {
        ...
        _initEvents: function() {
            ...
            $body.on('click', 'tr > td .mmg-check', function(e) {
                ...
                var $this = $(this);
                if (this.checked) {
                    //如果当前没有高亮行，则高亮本行，否则值做勾选处理
                    if (0 === that.selectedRowIndex().length) {
                        that.select($($this.parents('tr')[0]).index());
                    }
                } else {
                    that.deselect($($this.parents('tr')[0]).index());
                    //高亮当前已经勾选的行中的第一行，如果没有则都不高亮
                    var checkedTrIndex = that.checkedRowsIndex()[0];
                    if (undefined !== checkedTrIndex) {
                        that.select(checkedTrIndex);
                    }
            ...
            if (opts.checkCol) {
                $head.find('th .checkAll').on('click', function() {
                    if (0 === $body.find('tr > td').find('.mmg-check:enabled').length) {
                        return;
                    }
                    if (this.checked) {
                        that.select('all');
        ...
        _rowHtml: function(item, rowIndex) {
            ...
            if ($.isPlainObject(item)) {
                ...
                for (var colIndex = 0; colIndex < leafCols.length; colIndex++) {
                    ...
                    var val;
                    if (col.renderer) {
                        val = col.renderer(item[col.name], item, rowIndex);
                    } else {
                        val = "" === item[col.name] ? "&nbsp;" : item[col.name];
                    }
                    var title;
                    var needTitle = true;
                    var objTitle = col.rendererTitle;
                    //undefined 为使用render值，true为使用默认原始数据,function则执行回调,false则不需title，其他情况下使用该给定的值
                    if (undefined === objTitle) {
                        title = val;
                    } else if (true === objTitle) {
                        title = item[col.name];
                    } else if ('function' == typeof objTitle) {
                        title = objTitle(item[col.name], item, rowIndex);
                    } else if (false === objTitle) {
                        needTitle = false;
                    } else {
                        title = objTitle;
                    }
                    if (needTitle) {
                        trHtml.push('" title="' + title + '">');
                    } else {
                        trHtml.push('">');
                    }
        ...
        _fullWidthRows: function() {
            ...
            //请注意,在表格默认隐藏情况下,设置了fullWidthRows并且加载数据时thsArr为空,以下代码中会出错
            //在此加入此代码进行保护
            if (0 === thsArr.length) {
                return;
            }
        ...
        _loadAjax: function(args) {
            ...
            //新增参数转化为json格式
            params = top.Main.Ctrl.obj2json(params);
        ...
        _loadNative: function(args) {
            // var data = {};
            var data;
            if (0 !== this.opts.plugins.length) {
                // this.opts.plugins[0].
                var p = parseInt(this.opts.plugins[0].$el.data('page'), 10);
                var l = parseInt(this.opts.plugins[0].$limitList.val(), 10);

                data = {};
                var s = (p - 1) * l;
                var e = p * l;
                e = e < args.length ? e : args.length,
                data[this.opts.root] = args.slice(s, e);
                data[this.opts.plugins[0].opts.totalCountName] = args.length;
                args = data[this.opts.root];
            } else {
                data = args;
            }
            ...
            this.$body.triggerHandler('loadSuccess', data);
        },
        setparams: function(params, page) {
            //重设查询条件和查询页面
            this.opts.params = params;
            this.opts.plugins[0].$el.data('page', page);
        },
        load: function(args) {
            try {
                var opts = this.opts;
        ...
        reload: function(args) {
            ...
            if (this.opts.plugins.length > 0) {
                this.opts.plugins[0].$el.data('page', 1);
            }
        ...
        select: function(args) {
            ...
            if (typeof args === 'number') {
                ...
                if (!opts.multiSelect) {
                    ...
                    if (opts.checkCol) {
                        $body.find('tr > td').find('.mmg-check:enabled').prop('checked', '');
                    }
                }
                if (0 === this.selectedRowIndex().length) {
                    $tr.addClass('selected');

                }
                if (opts.checkCol) {
                    $tr.find('td .mmg-check:enabled').prop('checked', 'checked');
                }
                ...
                $.each($body.find('tr'), function(index) {
                    if (args($.data(this, 'item'), index)) {
                        ...
                        if (!$this.hasClass('selected')) {
                            ...
                            if (opts.checkCol) {
                                $this.find('td .mmg-check:enabled').prop('checked', 'checked');
                            }
                        }
                    }
                });
                ...
                $body.find('tr').each(function(index, item) {
                    if ($(this).find('.mmg-check:enabled').length > 0) {
                        that.select(index);
                    }
        ...
        //更新全选按钮
        updateCheckAllCbx: function() {
            var opts = this.opts;
            var $body = this.$body;
            var $head = this.$head;
            if (opts.checkCol) {
                var $checks = $body.find('tr > td').find('.mmg-check:enabled');
                if (0 === $checks.length) {
                    return;
                }
        ...
        //设置禁用复选框的索引，参数可以为两种类型，
        //1:number型，表示前N个禁用
        //1:Array型，为数组下标数组,下标从0开始，即可指定某些行禁用
        disableCbx: function(args) {
            var opts = this.opts;
            if (opts.checkCol) {
                var $body = this.$body;
                var $head = this.$head;
                if (typeof args === 'number') {
                    for (var i = 0; i < args; i++) {
                        $body.find('tr > td').find('.mmg-check').eq(i).attr("disabled", true);
                    }

                } else if (args instanceof Array) {
                    for (var i = 0, l = args.length; i < l; i++) {
                        $body.find('tr > td').find('.mmg-check').eq(args[i]).attr("disabled", true);
                    }
        ...
        checkedRows: function() {
            var $body = this.$body;
            var selected = [];
            $.each($body.find('tr').has('td .mmg-check:checked'), function(index, item) {
                selected.push($.data(this, 'item'));
            });
            return selected;
        },
        //获取当前高亮行的数据，目前返回形式为长度为1的对象数组
        ...
        //获取勾选的行的索引数据
        checkedRowsIndex: function() {
            var $body = this.$body;
            var $trs = this.$body.find('tr')
            var selected = [];
            $.each($body.find('tr').has('td .mmg-check:checked'), function(index) {
                selected.push($trs.index(this));
            });
            return selected;
        },
        //获取高亮行的索引，目前返回形式为长度为1的整数数组
    ...
    $.fn.mmGrid.defaults = {
        ...
        sortName: '',
        sortStatus: 'asc',
        loadingText: '' //正在载入时的提示
        ,
        noDataText: '' //没有数据时的提示信息
        ,
        loadErrorText: '' //数据异常时的提示
        ,
        multiSelect: false //是否支持勾选多行
        ,
        checkCol: false,
        indexCol: false,
        indexColWidth: 30,
